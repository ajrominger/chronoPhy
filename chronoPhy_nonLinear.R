...
RhoExpR <- function(rho0, b, S, t0) {
	...
	mapply(these.t=t0, these.rho=rho0, FUN=function(these.t, these.rho) {
		if(these.rho==0 | these.t < 0) {
			...
			# pfun <- function(x) 1 - exp(-rho0/b * (exp(-b*t0) - exp(-b*x)))
			# dfun <- function(x) these.rho * exp(-(b*x - these.rho/b * (exp(-b*these.t) - exp(-b*x))))
			ratefun <- function(x) these.rho * exp(-b*(x-these.t))
			browser()
			probfun <- AbscontDistribution(d=dfun, low=0, low1=0)
			probfun@r(100)
			rexp(100, 0.1)
			out <- AbscontDistribution(d=dfun, low=0, low1=0)@r(1)
		}
...
x <- evolve(LaFun=RhoExpR, mu0=0, bmu=0, nu0=0, ga0=0, nGen=10, rho0=la0, b=bla)
x
