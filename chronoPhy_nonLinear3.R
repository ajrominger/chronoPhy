...
RhoExpR <- function(rho0, b, S, t0) {
	...
	out <- rates
	out[rates > 0] <- rexp(sum(rates > 0), rates[rates > 0])
	out[rates == 0] <- NA
...
RhoQuadR <- function(rho0, b, S, t0) {
	# browser()
	rho0 <- rho0 * colSums(S, na.rm=TRUE)
	
	rates <- rho0 * exp(-b*(t0-1.5)^2)
	rates[t0 < 0] <- 0
	# plot(t0, rates)
...
event <- function(time, type, event.index) {
	if(type == 3) { # immigration
		...
		parent <- try(sample(which(S[, site] > 0), 1))
		if(inherits(parent, 'try-error')) browser()
