...
mean.stats.quad <- apply(island.stat[, c(1:3, 5:7), ], 2, function(x) apply(x, 1, mean, na.rm=TRUE))
se.stats.quad <- apply(island.stat[, 1:3, ], 2, function(x) apply(x, 1, sd, na.rm=TRUE))#/sqrt(sum(keepem))
